...
export function resolveRuleSet(
  ...
  const candidates = rows
    ...
    .sort((a, b) => toDate(b.effectiveFrom).getTime() - toDate(a.effectiveFrom).getTime());
...
export function validateNoOverlap(rows: RuleSetRow[]): void {
  ...
  for (const [year, list] of byYear) {
    const sorted = [...list].sort((a, b) => toDate(a.effectiveFrom).getTime() - toDate(b.effectiveFrom).getTime());
    for (let i = 0; i < sorted.length - 1; i++) {
      const a = sorted[i];
      const b = sorted[i + 1];
      if (toDate(a.effectiveFrom).getTime() === toDate(b.effectiveFrom).getTime()) {
        throw new OverlapError(`Overlap rule set tahun ${year}: ${a.version} dan ${b.version} memiliki effectiveFrom sama ${toDate(a.effectiveFrom).toISOString()}`);
      }

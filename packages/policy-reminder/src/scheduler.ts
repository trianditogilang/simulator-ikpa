...
export function planDeliveries(input: ScheduleInput): Array<{ scheduledFor: string; idempotencyKey: string; leadDays: number }> {
  const out: Array<{ scheduledFor: string; idempotencyKey: string; leadDays: number }> = [];
...
export async function insertScheduledDeliveries(
  ...
  for (const d of deliveries) {
    try {
      ...
      if (msg.includes("idempotency") || msg.includes("duplicate") || msg.includes("unique")) continue;
      throw e;
    }
  }
  return inserted;
...
export async function selectDueDeliveries(db: DbClient, nowIso: string, batchLimit = 50) {
  const now = new Date(nowIso);
  const rows = await db.select().from(notificationDeliveries).where(and(eq(notificationDeliveries.status, "scheduled"), lte(notificationDeliveries.scheduledFor, now))).limit(batchLimit);
  return rows;
}
...
export async function reEvaluatePending(
  ...
  const rows = await db.select().from(notificationDeliveries).where(and(eq(notificationDeliveries.status, "scheduled"))).limit(100);
  // ponytail: placeholder re-evaluation - caller will regenerate keys and cancel outdated
  return rows.filter(r => (r as unknown as { orgId?: string }).orgId === orgId || true).slice(0, 20);
}

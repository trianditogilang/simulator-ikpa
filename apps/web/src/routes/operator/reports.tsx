...
import { fetchOperatorXlsx, triggerDownload } from "@/services/report-service";
			...
			<div className="space-y-6">
				<div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-border bg-surface p-5 shadow-xs">
					<div>
						...
						<p className="text-xs text-muted-foreground">Ekspor data tabel Operator ke format Excel (XLSX). Data scoped per satker, filter tercantum, injection di-netralkan, dan file via download terautentikasi (tanpa URL publik permanen).</p>
					</div>
				</div>
				...
				<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
					<div className="flex flex-col justify-between rounded-2xl border border-border bg-background p-6 shadow-xs md:col-span-2">
						<div>
						...
						<div className="mt-6 border-t border-border/80 pt-4">
							<button type="button" disabled={generating !== null} onClick={handleDownload} className="w-full rounded-xl bg-primary py-2 text-center text-xs font-semibold text-primary-foreground shadow-xs transition hover:bg-primary-hover disabled:opacity-50">

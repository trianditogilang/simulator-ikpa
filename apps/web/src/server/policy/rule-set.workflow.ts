...
export async function createDraft(
  ...
  if (inv.length) throw new Error(`Invariant violation: ${inv.map(i => `${i.path}: ${i.message}`).join("; ")}`);

  const [created] = await db.insert(ruleSets).values({
    year: data.year,
...
export async function publishRuleSet(
  ...
  const [row] = await db.select().from(ruleSets).where(eq(ruleSets.id, ruleSetId)).limit(1);
  if (!row) throw new Error("Rule set tidak ditemukan");
  if (row.status !== "draft") throw new Error("Hanya draft yang dapat dipublish");
  if (!row.sourceRegulation || !row.changeNotes) throw new Error("sourceRegulation & changeNotes wajib sebelum publish");
  // check invariant again
  const inv = validateInvariants(row.configJson as never);
  if (inv.length) throw new Error(`Invariant blocked publish: ${inv.map(i => i.message).join("; ")}`);

  const [published] = await db.update(ruleSets).set({ status: "published", publishedAt: new Date(), updatedAt: new Date() }).where(eq(ruleSets.id, ruleSetId)).returning();
...
export async function retireRuleSet(
  ...
  const [row] = await db.select().from(ruleSets).where(eq(ruleSets.id, ruleSetId)).limit(1);
  if (!row) throw new Error("Rule set tidak ditemukan");
  if (row.status !== "published") throw new Error("Hanya published yang dapat di-retire");
  const [retired] = await db.update(ruleSets).set({ status: "retired", retiredAt: new Date(), updatedAt: new Date() }).where(eq(ruleSets.id, ruleSetId)).returning();
  await writeAudit(db, {
    actorId: meta.actorId,
    actorAccessType: "admin_kppn",
    entityType: "rule_sets",
...
export async function diffRuleSets(db: DbClient, aId: string, bId: string) {
  const [a] = await db.select().from(ruleSets).where(eq(ruleSets.id, aId)).limit(1);
  const [b] = await db.select().from(ruleSets).where(eq(ruleSets.id, bId)).limit(1);

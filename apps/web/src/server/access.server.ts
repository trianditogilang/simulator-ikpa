...
/**
 * Server function to resolve user access from current session.
 */
export async function getAccessResolutionForSession(
	...
	if (!dbUrl) {
		// Fallback for mock/dev environment without active DB connection
		...
		if (auth.clerkUserId.includes("multi")) {
			...
			const activeId = requestedOrgId === org2.id ? org2.id : (requestedOrgId === org1.id ? org1.id : null);
			return accessResolutionSchema.parse({

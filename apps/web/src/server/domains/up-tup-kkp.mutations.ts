...
export async function createUpTup(
	db: DbClient,
	access: AccessResolution,
	orgId: string,
	input: unknown,
...
export async function upsertKkp(
	db: DbClient,
	access: AccessResolution,
	orgId: string,
	input: unknown,
	...
	const [existing] = await db
		...
		.where(
			and(
				eq(kkpUsages.fiscalYearId, data.fiscalYearId),
				eq(kkpUsages.month, data.month),
				isNull(kkpUsages.deletedAt),
			),
		)
		.limit(1);
	if (existing) {
		const [updated] = await db
			...
			.where(eq(kkpUsages.id, existing.id))
			.returning();
		await writeAudit(db, {
			actorId: meta.actorId,
			actorAccessType: "operator_satker",
			entityType: "kkp_usages",
...
export async function softDeleteKkp(
	db: DbClient,
	access: AccessResolution,
	orgId: string,
	id: string,

...
async function assertFiscalYearAccess(
	...
	if (!fy || fy.orgId !== orgId)
		throw new Error("Fiscal year tidak ditemukan atau di luar scope.");
	return fy;
}
...
export async function listBudgets(
	...
	return db
		...
		.from(budgets)
		.where(
			and(eq(budgets.fiscalYearId, fiscalYearId), isNull(budgets.deletedAt)),
		);
}
...
export async function listRevisions(
	...
	return db
		...
		.where(
			and(
				eq(dipaRevisions.fiscalYearId, fiscalYearId),
...
export function previewRevisionEligibility(
	revisionCode: string,

...
export async function listSpmQ4(db: DbClient, access: AccessResolution, orgId: string, fiscalYearId: string) {
  await assertFy(db, access, orgId, fiscalYearId);
  return db.select().from(spmQ4).where(and(eq(spmQ4.fiscalYearId, fiscalYearId), isNull(spmQ4.deletedAt)));
}

export function previewDispensationRatio(dispensationCount: number, totalQ4: number, buckets: { minRatio: string; maxRatio: string; deduction: string }[]) {
  if (totalQ4 === 0) return { ratio: "0", deduction: "0" };
  const permil = (dispensationCount / totalQ4) * 1000;
  const bucket = buckets.find(b => permil >= parseFloat(b.minRatio) && permil <= parseFloat(b.maxRatio));

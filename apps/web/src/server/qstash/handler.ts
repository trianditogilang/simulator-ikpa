...
export async function handleQStashDaily(
	...
	const due = (await (db as never)
		...
		.limit(50)) as unknown as Array<{ id: string; scheduledFor: Date }>;
	let processed = 0;
			...
			try {
				...
				await db
					...
					.set({
						...
						errorMessage: "daily transition failed" as never,
						updatedAt: now as never,
					})
					.where(eq(notificationDeliveries.id, row.id));
			}
...
export async function handleQStashSend(
	...
	const rows = (await (db as never)
		...
		.from(notificationDeliveries)
		.where(eq(notificationDeliveries.status, "scheduled"))
		.limit(batchLimit)) as unknown as Array<{
		id: string;
		...
		try {
			...
			await db
				...
				.set({
					status: "sent" as never,
					sentAt: now as never,
					attemptCount: (row.attemptCount + 1) as never,
					updatedAt: now as never,
				})
				.where(eq(notificationDeliveries.id, row.id));
			sent++;

	...
	return db
		...
		.where(
			and(
				...
				eq(orgReminderConfigs.fiscalYearId, fiscalYearId),
			),
		);
}
...
export async function previewReminderSchedule(
	...
	configOverride?: {
		...
		additionalRecipientsJson?: unknown;
	},
	...
	if (!policy.isActive)
		throw new Error("Policy tidak aktif, tidak dapat di-preview.");
	...
	const schedule = (configOverride?.scheduleJson ??
		policy.defaultScheduleJson) as { leadDays?: number[]; sendHour?: number };
	...
	return {
		...
		scheduled,
		requiredRecipients: policy.requiredRecipientsJson,
	};
}

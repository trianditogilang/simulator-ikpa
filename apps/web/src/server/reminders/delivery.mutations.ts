...
export async function retryFailedDelivery(
  ...
  if (del.status !== "failed") throw new Error("Hanya delivery dengan status failed yang dapat di-retry.");
  ...
  const newPayload = { ...payload, _attemptTrace: [...((payload._attemptTrace as unknown[]) ?? []), { attempt: nextAttempt, retriedAt: new Date().toISOString(), actorId: meta.actorId }] };
  ...
  }).where(eq(notificationDeliveries.id, deliveryId)).returning();

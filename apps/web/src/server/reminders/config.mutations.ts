...
export async function upsertReminderConfig(
  ...
  const [policy] = await db.select().from(reminderPolicies).where(eq(reminderPolicies.id, data.reminderPolicyId)).limit(1);
  if (!policy) throw new Error("Policy tidak ditemukan.");
  if (!policy.isActive) throw new Error("Policy tidak aktif, tidak dapat dikonfigurasi.");

  // compliance guard
  const scheduleLeadDays = (data.scheduleJson as { leadDays?: number[] })?.leadDays;
  const errors = checkCompliance(
    ...
    },
    {
      ...
      scheduleLeadDays,
      recipients: [...((policy.requiredRecipientsJson as string[]) ?? []), ...((data.additionalRecipientsJson) ?? [])],
    },
  );
  if (errors.length) throw new Error(errors.map((e: { message: string }) => e.message).join("; "));

  const [existing] = await db.select().from(orgReminderConfigs).where(and(eq(orgReminderConfigs.orgId, orgId), eq(orgReminderConfigs.fiscalYearId, data.fiscalYearId), eq(orgReminderConfigs.reminderPolicyId, data.reminderPolicyId))).limit(1);

  const sanitized = sanitizeMessage(data.customMessage ?? null);

  if (existing) {
    const [updated] = await db.update(orgReminderConfigs).set({
      enabled: data.enabled,
      ...
      updatedAt: new Date(),
    }).where(eq(orgReminderConfigs.id, existing.id)).returning();
  ...
  const [created] = await db.insert(orgReminderConfigs).values({
    orgId,
...
export async function resetReminderConfigToDefault(
  ...
  const [policy] = await db.select().from(reminderPolicies).where(eq(reminderPolicies.id, cfg.reminderPolicyId)).limit(1);
  if (!policy) throw new Error("Policy tidak ditemukan.");
  ...
  const [updated] = await db.update(orgReminderConfigs).set({
    ...
    updatedBy: meta.actorId,
    updatedAt: new Date(),
  }).where(eq(orgReminderConfigs.id, configId)).returning();

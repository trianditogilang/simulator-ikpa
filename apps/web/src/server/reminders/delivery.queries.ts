...
export async function listDeliveriesForAdmin(
	...
	const whereClauses: unknown[] = [
		inArray(notificationDeliveries.orgId, orgIds as never),
	];
	if (filter.status)
		whereClauses.push(
			eq(notificationDeliveries.status, filter.status as never),
		);
	if (filter.orgId) {
		if (!orgIds.includes(filter.orgId))
			throw new Error("Org di luar scope admin");
		whereClauses.push(eq(notificationDeliveries.orgId, filter.orgId as never));
	}
	const where =
		whereClauses.length === 1
			? (whereClauses[0] as never)
			: (and(...(whereClauses as never[])) as never);
	...
	const totalRows = (await db
		.select()
		.from(notificationDeliveries)
		.where(where)) as unknown as unknown[];
	return {
		items,

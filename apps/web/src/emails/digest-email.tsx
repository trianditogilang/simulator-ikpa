			...
			>
				...
				>
					<h1 style={{ fontSize: 16, fontWeight: 700 }}>
						Ringkasan Harian IKPA — {props.date}
					</h1>
					<p style={{ fontSize: 13, color: "#64748b" }}>
						{props.satkerCode} — {props.satkerName} • v{props.ruleSetVersion}
					</p>
						...
						(prio) =>
							grouped[prio].length > 0 && (
								<div key={prio} style={{ marginTop: 12 }}>
									...
									>
										{prio} priority ({grouped[prio].length})
									</h3>
									<ul style={{ fontSize: 12, paddingLeft: 16 }}>
										{grouped[prio].map((it) => (
											<li
												key={`${prio}-${it.eventType}-${it.deadline}-${it.actionLabel}`}
											>
												{it.eventType} — deadline {it.deadline} —{" "}
												{it.actionLabel}
											</li>
										))}
									</ul>
					...
					<p style={{ fontSize: 11, color: "#94a3b8", marginTop: 12 }}>
						Email diges tidak mengandung data sensitif. Detail lengkap di
						aplikasi.
					</p>
				</div>
			</body>
...
export function digestEmailText(props: DigestEmailProps): string {
	return (
		`Digest ${props.date} ${props.satkerCode}\n` +
		props.items
			.map(
				(i) =>
					`${i.priority.toUpperCase()} ${i.eventType} deadline ${i.deadline}`,
			)
			.join("\n") +

...
export function ReminderEmail(props: ReminderEmailProps) {
	const safeCustom = props.customMessage
		? escapeHtml(props.customMessage)
		: null;
	const link = props.secureLink.startsWith("https://")
		? props.secureLink
		: `https://${props.secureLink}`;
	return (
		<html lang="id">
			<body
				style={{
			...
			>
				...
				>
					<h1 style={{ fontSize: 18, fontWeight: 700, margin: "0 0 8px" }}>
						Pengingat IKPA: {props.eventType}
					</h1>
					...
					>
						<tbody>
							<tr>
								<td style={{ padding: "6px 0", color: "#64748b" }}>Deadline</td>
								<td style={{ padding: "6px 0", fontWeight: 600 }}>
									{props.deadline} ({props.dayType}){" "}
									{props.leadDays !== undefined ? `H-${props.leadDays}` : ""}
								</td>
							</tr>
							<tr>
								<td style={{ padding: "6px 0", color: "#64748b" }}>Aksi</td>
								<td style={{ padding: "6px 0" }}>{props.actionLabel}</td>
							</tr>
							<tr>
								<td style={{ padding: "6px 0", color: "#64748b" }}>Sumber</td>
								<td style={{ padding: "6px 0" }}>
									{props.sourceRegulation} v{props.ruleSetVersion}
								</td>
							</tr>
						</tbody>
					</table>
					{safeCustom && (
						<div
							...
							// biome-ignore lint/security/noDangerouslySetInnerHtml: customMessage is escaped before rendering in this email template.
							dangerouslySetInnerHTML={{ __html: safeCustom }}
						/>
					)}
					<a
						href={link}
					...
					<p style={{ fontSize: 11, color: "#94a3b8", marginTop: 16 }}>
						Link aman akan kadaluarsa dalam 24 jam. Jangan bagikan link ini.
					</p>
				</div>
			</body>
		</html>

...
## 15. Fase 11 â€” Integrasi Frontendâ€“Backend Bertahap
...
- [x] **CORR-A-00 — Bekukan kontrak: Admin monitor 8 indikator + reminder wajib (parkir).** [Role: Frontend Admin Agent]
...
- [x] **CORR-A-01 — Dashboard Admin agregat 8 baris + deadline wajib (parkir).** [Role: Frontend Admin Agent]
  **Depends:** CORR-01..05 [x] + docs/operator-freeze.md
  **Syarat:** CORR-02..04
  **DoD:** Read-only. Tanpa sel kuning. `assertAdminKppnScope` di setiap query. Tanpa mutasi operasional.

- [x] **CORR-A-02 — Daftar satker: 8 skor, gap, actual vs proyeksi (parkir).** [Role: Frontend Admin Agent]
  **Depends:** CORR-01..05 [x] + docs/operator-freeze.md, CORR-A-01
  **Syarat:** Loader bukan mock
  **DoD:** Read-only. Tanpa sel kuning. `assertAdminKppnScope` di setiap query. Tanpa mutasi operasional.

- [x] **CORR-A-03 — Detail satker read-only `/admin-kppn/organizations/:orgId` (parkir).** [Role: Frontend Admin Agent]
  **Depends:** CORR-01..05 [x] + docs/operator-freeze.md, CORR-A-02
  **DoD:** Read-only. Tanpa sel kuning. `assertAdminKppnScope` di setiap query. Tanpa mutasi operasional.

- [x] **CORR-A-04 — Monitoring reminder mandatory (parkir).** [Role: Frontend Admin Agent]
  **Depends:** CORR-01..05 [x] + docs/operator-freeze.md
  **Syarat:** Policy backend ada
  **DoD:** Read-only. Tanpa sel kuning. `assertAdminKppnScope` di setiap query. Tanpa mutasi operasional.

- [x] **CORR-A-05 — Policy, kalender, akses, audit tetap (parkir).** [Role: Frontend Admin Agent]
  **Depends:** CORR-01..05 [x] + docs/operator-freeze.md, CORR-A-01..03
  **DoD:** Read-only pada data transaksi operasional satker (tanpa sel kuning, tanpa mutasi operasional). Mutasi administratif yang sah (policy/rule set, mapping akses, retry delivery) server-authoritative, dibatasi `assertAdminKppnScope`, teraudit, dan proteksi admin aktif terakhir ditegakkan. Selesai 2026-09-09.
...
|---|---|---|
| M0 â€” Decision Ready | Fase 0 | ADR, kontrak, dan status regulasi cukup untuk membangun tanpa asumsi tersembunyi |
| M1 â€” UI Prototype | Fase 1â€“4 | Seluruh area publik, Operator, dan Admin dapat didemokan dengan dummy data |
| M2 â€” UI Accepted | Fase 5 | Stakeholder menerima navigasi, responsivitas, state, dan alur utama |
| M3 â€” Domain Ready | Fase 6 | Engine deterministik dan golden tests lulus |
| M4 â€” Backend Ready | Fase 7â€“10 | DB, auth, domain services, policy, scheduler, dan email tersedia |
...
- [TanStack Start â€” routing](https://tanstack.com/start/latest/docs/framework/react/guide/routing): file-based routes berada di `src/routes` dan route tree dihasilkan oleh tooling.
- [TanStack Start â€” middleware](https://tanstack.com/start/latest/docs/framework/react/guide/middleware): global request middleware dikonfigurasi melalui `createStart` di `src/start.ts`.
- [TanStack Start â€” server functions](https://tanstack.com/start/latest/docs/framework/react/guide/server-functions): loader dan komponen dapat memanggil server functions; otorisasi tetap server-authoritative.
